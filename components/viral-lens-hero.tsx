      ...
      <div className="relative px-6 py-16">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-neon-blue via-neon-purple to-neon-pink bg-clip-text text-transparent leading-tight">
              See What's Going Viral
              <br />
              <span className="text-4xl md:text-6xl">Before Everyone Else</span>
            </h1>
          ...
          <div className="max-w-2xl mx-auto">
            <form onSubmit={handleHeroSearch} className="relative">
              <div className="relative group">
                <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground group-focus-within:text-neon-blue transition-colors" />
                <Input
                  placeholder="Ask ViralLens about a topic..."
                  value={heroSearch}
                  onChange={(e) => setHeroSearch(e.target.value)}
                  className="h-14 pl-12 pr-32 text-lg bg-card/50 border-border/50 focus:border-neon-blue focus:ring-neon-blue/20 focus:bg-card backdrop-blur-sm"
                />
                <Button
                  type="submit"
                  disabled={isSearching}
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-10 px-6 bg-gradient-to-r from-neon-blue to-neon-purple hover:from-neon-purple hover:to-neon-pink neon-glow-blue transition-all duration-300"
                >
                  {isSearching ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
            {[
              ...
              <Card
                key={index}
                className="p-6 bg-card/30 backdrop-blur-sm border-border/50 hover:border-neon-blue/30 transition-all duration-300 group"
              >
                <div className="text-center space-y-2">
                  <div className="text-2xl font-bold text-foreground group-hover:text-neon-blue transition-colors">
                    {stat.value}
                  </div>
